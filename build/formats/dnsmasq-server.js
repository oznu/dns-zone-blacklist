...
module.exports = (master) => {
  const dest = path.resolve(__dirname, '../../dnsmasq/dnsmasq-server.blacklist')
  ...
  .then((bind) => {
    ...
    fs.writeFile(checksum, sha256, (err) => {
      ...
      console.log(`dnsmasq-server checksum is ${sha256}`)
    })
    ...
    fs.writeFile(dest, bind, (err) => {
      ...
      console.log(`${master.length} dnsmasq-server zones saved to ${dest}`)
    })
  })
}
